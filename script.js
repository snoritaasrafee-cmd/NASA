...
for(let i=0;i<15;i++){
  ...
  f.style.width=12+Math.random()*12+"px";
  f.style.height=12+Math.random()*12+"px";
...
async function fetchData(){
  ...
  try{
    ...
    allData=[
      {species:"Cherry Blossom",date:"2025-03-28",lat:35.68,lng:139.69,ndvi:0.65},
...
function updateTopSpecies(){
  ...
  sorted.forEach(s=>{
    ...
    span.className="pred-item";
    topSpeciesList.appendChild(span);
  });
}

function predictNextBloom(){
  return allData.map(d=>{
    let next=new Date(d.date); next.setDate(next.getDate()+7);
    return `${d.species} around ${next.toISOString().split("T")[0]}`;
  });
}
...
exportCSV.addEventListener("click",()=>{
  ...
  a.href=url; a.download="bloom_data.csv"; a.click();
...
playTimelineBtn.addEventListener("click",()=>{
  ...
  const dates=[...new Set(allData.map(d=>d.date))].sort((a,b)=>new Date(a)-new Date(b));
  animIndex=0;
  playTimelineBtn.textContent="⏸ Pause"; isPlaying=true;
  animationInterval=setInterval(()=>{
    ...
    renderCharts(); updateTopSpecies(); updateHotspot();
    animIndex++;
  },800);
});
